...
# Check for the occurrence of the substring 'n/a' in the entire sheet
na_occurrences <- apply(dirty_data_NC, 1, function(row) any(grepl("n/a", as.character(row), ignore.case = TRUE)))

# Subset the data frame to keep only rows without 'n/a' occurrences
dirty_data_NC <- dirty_data_NC[!na_occurrences, ]

#Dropping the columns with Random values
dirty_data_NC <- dirty_data_NC[, -which(names(dirty_data_NC) == "deceased_indicator")]


library(dplyr)

dirty_data_NC<-dirty_data_NC %>%
